...
private constructor(
     ...
     * ```java
     * MyClass myObject = spendCalculateSpendParams.completionResponse().convert(MyClass.class);
     * ```
     */
    ...
    private constructor(
         ...
         * This arbitrary value can be deserialized into a custom type using the `convert` method:
         * ```java
         * MyClass myObject = body.completionResponse().convert(MyClass.class);
         * ```
         */
        @JsonProperty("completion_response")
        @ExcludeMissing
        ...
        internal fun validity(): Int =
            (messages.asKnown().getOrNull()?.size ?: 0) + (if (model.asKnown().isPresent) 1 else 0)
