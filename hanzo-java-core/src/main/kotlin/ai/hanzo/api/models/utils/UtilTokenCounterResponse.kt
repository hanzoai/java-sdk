...
private constructor(
     ...
     *   server responded with an unexpected value).
     */
    fun originalResponse(): Optional<OriginalResponse> =
        originalResponse.getOptional("original_response")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.originalResponse] with `originalResponse.orElse(null)`. */
        fun originalResponse(originalResponse: Optional<OriginalResponse>) =
            originalResponse(originalResponse.getOrNull())
    ...
    fun validate(): UtilTokenCounterResponse = apply {
        ...
        error()
        errorMessage()
        originalResponse().ifPresent { it.validate() }
        statusCode()
        validated = true
    }
    ...
    internal fun validity(): Int =
        (if (modelUsed.asKnown().isPresent) 1 else 0) +
            (if (requestModel.asKnown().isPresent) 1 else 0) +
            (if (tokenizerType.asKnown().isPresent) 1 else 0) +
            (if (totalTokens.asKnown().isPresent) 1 else 0) +
            (if (error.asKnown().isPresent) 1 else 0) +
            (if (errorMessage.asKnown().isPresent) 1 else 0) +
            (originalResponse.asKnown().getOrNull()?.validity() ?: 0) +
