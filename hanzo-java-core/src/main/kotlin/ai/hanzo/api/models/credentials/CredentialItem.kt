...
private constructor(
     ...
     * ```java
     * MyClass myObject = credentialItem.credentialInfo().convert(MyClass.class);
     * ```
     */
    @JsonProperty("credential_info")
    @ExcludeMissing
     ...
     * This arbitrary value can be deserialized into a custom type using the `convert` method:
     * ```java
     * MyClass myObject = credentialItem.credentialValues().convert(MyClass.class);
     * ```
     */
    @JsonProperty("credential_values")
    @ExcludeMissing
     ...
     */
    @JvmSynthetic internal fun validity(): Int = (if (credentialName.asKnown().isPresent) 1 else 0)
