...
private constructor(
     ...
     *   server responded with an unexpected value).
     */
    fun auth(): Optional<Boolean> = auth.getOptional("auth")
     ...
     *   server responded with an unexpected value).
     */
    fun costPerRequest(): Optional<Double> = costPerRequest.getOptional("cost_per_request")
     ...
     *   server responded with an unexpected value).
     */
    fun guardrails(): Optional<Guardrails> = guardrails.getOptional("guardrails")
     ...
     *   server responded with an unexpected value).
     */
    fun includeSubpath(): Optional<Boolean> = includeSubpath.getOptional("include_subpath")
    ...
    class Builder internal constructor() {
        ...
        fun guardrails(guardrails: Optional<Guardrails>) = guardrails(guardrails.getOrNull())
    ...
    fun validate(): PassThroughGenericEndpoint = apply {
        ...
        auth()
        costPerRequest()
        guardrails().ifPresent { it.validate() }
    ...
    internal fun validity(): Int =
        (if (path.asKnown().isPresent) 1 else 0) +
            ...
            (if (costPerRequest.asKnown().isPresent) 1 else 0) +
            (guardrails.asKnown().getOrNull()?.validity() ?: 0) +
            (headers.asKnown().getOrNull()?.validity() ?: 0) +
            (if (includeSubpath.asKnown().isPresent) 1 else 0)
